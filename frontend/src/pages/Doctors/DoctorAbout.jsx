...
const DoctorAbout = ({name, about, qualifications, experiences}) => {
  return (
    <div>
        ...
        <div className='mt-12'>
            <h3 className='text-[20px] leading-[30px] text-headingColor font-semibold'>Образование</h3>
        ...
        <div className='mt-12'>
        <h3 className='text-[20px] leading-[30px] text-headingColor font-semibold'>Опыт</h3>
        <ul className='grid sm:grid-cols-2 gap-[30px] pt-4 md:p-5'>
            {experiences?.map((item,index)=> <li key={index} className='p-4 rounded bg-[#fff9ea]'>
                ...
                <p className='text-[15px] leading-6 font-medium text-textColor'>{item.hospital}</p>
            </li>)}
        </ul>
        </div>
